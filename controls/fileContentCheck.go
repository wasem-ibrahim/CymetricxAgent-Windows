...
func fileContentCheck(filePath, presenceRegex, expectRegex string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("invalid presence regex: %w", err)
	}

	expectRe, err := regexp.Compile(expectRegex)
	if err != nil {
		return false, fmt.Errorf("invalid expect regex: %w", err)
	}
	...
	for scanner.Scan() {
		...
		if presenceRe.MatchString(line) {
			...
			return false, fmt.Errorf("line matches presence regex but does not match expect regex")
		}
	}
	...
	return false, fmt.Errorf("presence regex not found in file")
}

// Handler functions for different types
func GetFileContentCheck(obj map[string]string, variables map[string]string) (map[string]string, error) {
